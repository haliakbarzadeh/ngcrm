...
namespace NgCrm.BasicInfoService.Application.Workspaces.Commands
{
    public class UpdateWorkspaceCommandValidator : AbstractValidator<UpdateWorkspaceCommand>
    {
        ...
        public UpdateWorkspaceCommandValidator(IWorkspaceQueryRepository workspaceQueryRepository)
        {
            _workspaceQueryRepository = workspaceQueryRepository;
        }

        public override async Task<ValidationResult> ValidateAsync(ValidationContext<UpdateWorkspaceCommand> context, CancellationToken cancellation = default)
        {
