...
{
    public class CreateUserCommand : BaseCommandRequest, IRequest<bool>
    {
    ...
    public class CreateUserCommandHandler : IRequestHandler<CreateUserCommand, bool>
    {
        ...
        public async Task<bool> Handle(CreateUserCommand request, CancellationToken cancellationToken)
        {
            ...
            var user = new User(request.PersonId, request.Username, request.Description, request.Password, request.IsActive);
            ...
            if (request.ADUserId is not null)
            {
                ...
                user.SetADUser(adUser.UserName, adUser.Id, adUser.IsActive);
            }

            _userCommandRepository.Add(user);

            if (request.AccessGroupIds is not null)
                _personAccessGroupCommandRepository.AddRange(request.AccessGroupIds.Distinct().Select(e => new PersonAccessGroup(request.PersonId, e)).ToList());
        ...
        private async Task<List<string>> CheckBusinessRules(CreateUserCommand request, CancellationToken cancellationToken)
        {
            ...
            var user = await _userQueryRepository.GetByExpressionAsync(u => u.PersonId == request.PersonId, cancellationToken);
            if (user is not null)
            ...
            {
                var userbyADUserId = await _userQueryRepository.GetByExpressionAsync(u => u.ADUserId == request.ADUserId, cancellationToken);
